...
template<typename T>
void Pharmacy<T>::checkExpiration() {
    T code;
    ...
    for (const auto& drug : drugs)
     {
        if (drug.code == code) {
            ...
            if (drug.endDate.year < currentDate.year ||
                (drug.endDate.year == currentDate.year && drug.endDate.month < currentDate.month) ||
                (drug.endDate.year == currentDate.year && drug.endDate.month == currentDate.month && drug.endDate.day < currentDate.day)) {
                cout << "The drug has expired." << endl;
            } 
            
            else
            
             {
                cout << "The drug has not expired." << endl;
            }
